...
def most_goals(player: list):
    max_goal = max(player, key=lambda rec: rec.goals)
    return max_goal.name

def most_points(player: list):
    max_passes = max(player, key=lambda rec: rec.passes)
    return (max_passes.name, max_passes.number)

def least_minutes(player: list):
    return min(player, key=lambda rec: rec.minutes)
